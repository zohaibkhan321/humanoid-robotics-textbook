...
def test_answer_endpoint_basic_functionality(mock_agent_service):
    """Test that the answer endpoint returns proper response structure"""
...
def test_answer_endpoint_with_selected_text(mock_agent_service):
    """Test that the answer endpoint works with selected text"""
    # Mock the agent service response
    mock_agent_service.process_query = AsyncMock(return_value={
...
def test_answer_endpoint_validation():
    """Test that the answer endpoint validates input properly"""
    response = client.post(
        "/api/v1/answer",
        json={
...
def test_embed_endpoint():
    """Test that the embed endpoint processes content"""
